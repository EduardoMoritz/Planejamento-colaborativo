# Projeto Colaborativo

## **18/03**

## Tarefas Realizadas

### Função de AD

1. ~~Função "Anúncios" para que os usuários recebam anúncios e a plataforma consiga lucrar em cima~~

### Conta Premium 

1. ~~Aba de conta premium para o usuário assinar~~
2. ~~Backend por trás do premium, vinculo com a função ad, e futuramente pagamentos~~

### Banco de Dados

1. ~~Criação do banco de dados e vínculo com funções~~

### Pagamentos

1. ~~Criação da aba para pagamentos~~
2. ~~Back e Banco de dados dos pagamentos, vínculo com função premium~~

## **19/03**

### Agenda

1. ~~Aba agenda para marcar atividades para outros dias~~
2. Back agend, incompleto

### Bloquear AntiAD

1. ~~Função de bloquear Bloqueador de anúncio, para gerar lucro pro app~~

### Atividade em conjunto

1. ~~Criação do Front atividade em conjunto~~

## 20/03

### Agenda

1. ~~Função agenda finalmente pronto~~

## 21/03

### Compartilhar 

1. ~~função compartilhar dados e atividades~~

### Central de Ajuda

1. ~~Criação Aba central de ajudas~~
2. ~~Vincular números e emails com a aba central de ajuda~~

### Atividade em conjunto

1. ~~Função atividade em conjunto pra poder criar grupos~~

## 22/03

### Modelos

1. ~~Pode criar modelos prontos e divulgar~~
2. ~~Pode copiar os modelos prontos pra sua biblioteca e acessar eles~~

## 23/03

1. Pequenos ajustes, atualizações e reviews, esperando feedbacks





